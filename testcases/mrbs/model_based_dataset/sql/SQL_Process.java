...
public class SQL_Process {
    ...
    String dbName = "";
    String version = "";
    int id = new Random(System.currentTimeMillis()).nextInt(850) + 10;
    ...
    public void addEntry(int room_id, String name, String description) throws SQLException {
        if (version.equals("old")){
            String sql = "INSERT INTO `" + dbName + "`.`" +
                    "mrbs_entry` (`id`, `start_time`, `end_time`, `entry_type`, `repeat_id`, `room_id`, `timestamp`, `create_by`, `name`, `type`, `desc" +
                    "ription`)" +
                    " VALUES ('" + id + "', '1674880000', '1696906900', '0', '0', " +
                    "'" + room_id + "', CURRENT_TIMESTAMP, 'administrator', '" + name + "', 'E', '" + description + "');";
            stmt.execute(sql);
            return;
        }
        String sql = "INSERT INTO `" + dbName + "`.`" +
                "mrbs_entry` (`id`, `start_time`, `end_time`, `entry_type`, `repeat_id`, `room_id`, `timestamp`, `create_by`, `name`, `type`, `desc" +
                "ription`, `status`, `reminded`, `info_time`, `info_user`, `info_text`, `ical_uid`, `ical_sequence`, `ical_recur_id`)" +
                " VALUES ('" + id + "', '1674880000', '1696906900', '0', '0', " +
                "'" + room_id + "', CURRENT_TIMESTAMP, 'administrator', '" + name + "', 'E', '" + description + "', '0', NULL, NULL, NULL, NULL, '', '0', '');";
        stmt.execute(sql);
    }
    ...
    public void addRoom(String name, String cap, String area_id, int id) throws SQLException {
        String sql = "INSERT INTO `" + dbName + "`.`mrbs_room`" +
                " (`id`, `area_id`, `room_name`, `description`," +
                " `capacity`, `room_admin_email`) " +
                "VALUES ('" + id + "', '" + area_id + "', '" + name + "'," +
